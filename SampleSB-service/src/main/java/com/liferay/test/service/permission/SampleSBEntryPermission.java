...
public class SampleSBEntryPermission implements BaseModelPermissionChecker {

	public static void check(PermissionChecker permissionChecker, SampleSB entry, String actionId)
			throws PortalException {

		if (!contains(permissionChecker, entry, actionId)) {
			throw new PrincipalException.MustHavePermission(permissionChecker, SampleSB.class.getName(),
	...
	public static void check(PermissionChecker permissionChecker, long entryId, String actionId)
		...
		if (!contains(permissionChecker, entryId, actionId)) {
			throw new PrincipalException.MustHavePermission(permissionChecker, SampleSB.class.getName(), entryId,
					actionId);
		}
	}

	public static boolean contains(PermissionChecker permissionChecker, SampleSB entry, String actionId) {

		String portletId = PortletProviderUtil.getPortletId(BlogsEntry.class.getName(), PortletProvider.Action.EDIT);

		Boolean hasPermission = StagingPermissionUtil.hasPermission(permissionChecker, entry.getGroupId(),
				SampleSB.class.getName(), entry.getSamplesbId(), portletId, actionId);
		...
		if (permissionChecker.hasOwnerPermission(entry.getCompanyId(), SampleSB.class.getName(), entry.getSamplesbId(),
				entry.getUserId(), actionId)) {

			return true;
		}

		return permissionChecker.hasPermission(entry.getGroupId(), SampleSB.class.getName(), entry.getSamplesbId(),
				actionId);
	}
